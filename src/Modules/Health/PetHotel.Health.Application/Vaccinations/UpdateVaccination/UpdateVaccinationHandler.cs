...
/// <summary>Carrega a ficha do pet no tenant corrente e edita a vacinação informada.</summary>
public static class UpdateVaccinationHandler
{
    public static async Task<Result> Handle(
        UpdateVaccination command,
        IValidator<UpdateVaccination> validator,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var result = record.UpdateVaccination(
            new VaccinationId(command.VaccinationId), command.Type, command.AppliedOn, command.ExpiresOn, today);
        if (result.IsFailure)
        {
            return result.Error;
        }
